...
def test_signup_creates_user_and_token(monkeypatch) -> None:
    ...
    response = client.post(
        "/api/auth/signup",
        json={"email": "Owner@Example.com", "password": "correct horse battery", "full_name": "Store Owner"},
    )
...
def test_signup_rejects_duplicate_email(monkeypatch) -> None:
    monkeypatch.setenv("USE_CLICKHOUSE", "false")
    client = TestClient(app)
    body = {"email": "owner@example.com", "password": "correct horse battery", "full_name": ""}
...
def test_login_rejects_wrong_password(monkeypatch) -> None:
    ...
    client.post(
        "/api/auth/signup",
        json={"email": "owner@example.com", "password": "correct horse battery", "full_name": ""},
    )

    response = client.post("/api/auth/login", json={"email": "owner@example.com", "password": "wrong password"})
